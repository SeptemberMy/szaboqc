...
若想充分了解Hartree-Fock理论, 那么可以阅读这一节. 本节先介绍泛函变分, 然后用这个级数来最小化单行列式的能量. 
...
Fock算符$f(1)$就是芯哈密顿算符$h(1)$加上如下的有效单电子势算符(称作Hartree-Fock势)：
\begin{align}
	\label{3.18}
...
\begin{align}
	\label{3.114}
	\left[ \int\dd\omega_1\alpha^*(\omega_1)f(\mathbf{x}_1)\alpha(\omega_1) \right]\psi_j(\mathbf{r}_1) = \epsilon_i\psi_j(\mathbf{r}_1)
\end{align}
...
Roothaan\endnote{
	C. C. J. Roothaan, New Development in molecular orbital theory, \textit{Rev. Mod. Phys.} \textbf{23:} 69 (1951).
}
...
也就是(近似)的分子轨道收敛到了\autoref{3.132}中的(精确)分子轨道——Fock算符的真实本征函数. 
对任意的有限基组. 
...
\begin{equation}
	\begin{split}
		...
		(\phi_2\phi_1|\phi_1\phi_1) & =(\phi_2\phi_2|\phi_2\phi_1) = 0.4441\au\\
