...
\begin{align}\label{4.52}
	\ket{\Psi_\mathrm{GVB}} = (2(1+S^2))^{-1/2} [u(1)v(2) + u(2)v(1)]2^{-1/2}(\alpha(1)\beta(2)-\alpha(2)\beta(1))
\end{align}
...
考虑如下变换
\begin{align*}
	u &= (a^2 + b^2)^{-1/2} (a\psi_A + b\psi_B) \\
	v &= (a^2 + b^2)^{-1/2} (a\psi_A - b\psi_B) 
\end{align*}

a. 证明 $\braket{u|u} = \braket{v|v} = 1$ 和 $\braket{u|v} = S = \dfrac{a^2 - b^2}{a^2 + b^2} $。

b. 证明\autoref{4.52}中的 $\ket{\Psi_\mathrm{GVB}}$ 可写为
\begin{equation*}
	\ket{\Psi_\mathrm{GVB}} = (a^4 + b^4)^{-1/2} [a^2\psi_A(1)\psi_A(2) - b^2\psi_B(1)\psi_B(2)]2^{-1/2}(\alpha(1)\beta(2)-\alpha(2)\beta(1))
\end{equation*}
并验证当
\begin{align*}
	c_A &= (a^4 + b^4)^{-1/2} a^2 \\
	c_B &= -(a^4 + b^4)^{-1/2} b^2 
\end{align*}
时，其与 \autoref{4.48} 的 $\ket{\Psi_\mathrm{MCSCF}}$ 等价。
}
